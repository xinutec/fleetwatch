//! The login-in-progress cookie. The case that matters is the one that broke the
//! Android wrapper: Nextcloud's Login Flow returns an EMPTY `state`, and a login must
//! still complete — while everything that isn't a login we started must not.

use chrono::{Duration, Utc};
use fleetwatch::pending_login::{accept, issue, ttl};
...
fn a_login_completes_when_nextcloud_echoes_the_state() {
    let now = now();
    let (nonce, cookie) = issue(SECRET, Some("/problems".into()), now);
    let pending = accept(SECRET, Some(&cookie), Some(&nonce), now).expect("accepted");
    assert_eq!(pending.return_to.as_deref(), Some("/problems"));
}
...
fn a_login_completes_when_the_login_flow_swallowed_the_state() {
    ...
    let now = now();
    let (_nonce, cookie) = issue(SECRET, Some("/problems".into()), now);
...
fn a_return_to_containing_the_field_separator_survives() {
    let now = now();
    let (nonce, cookie) = issue(SECRET, Some("/history?q=a|b".into()), now);
    let pending = accept(SECRET, Some(&cookie), Some(&nonce), now).expect("accepted");
    assert_eq!(pending.return_to.as_deref(), Some("/history?q=a|b"));
}
